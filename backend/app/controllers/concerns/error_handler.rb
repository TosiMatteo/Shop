module ErrorHandler
  ...
  def handle_not_found(exception)
    render_error(status: :not_found, message: "Risorsa non trovata")
  end

  # 422 - validation or persistence failures
  def handle_unprocessable_entity(exception)
    render_error(status: :unprocessable_entity, message: "Validazione fallita", details: exception.record.errors.full_messages)
  end

  # 400 - required parameters missing
  def handle_bad_request(exception)
    render_error(status: :bad_request, message: "Parametro obbligatorio mancante: #{exception.param}")
  end

  # 401 - authentication missing/invalid
  def handle_unauthorized(_exception)
    render_error(status: :unauthorized, message: "Autenticazione richiesta")
  end

  # 403 - authenticated but not allowed
  def handle_forbidden(_exception)
    render_error(status: :forbidden, message: "Accesso negato")
  end

  # 500 - fallthrough for unexpected errors
  def handle_internal_server_error(exception)
    Rails.logger.error("#{exception.class}: #{exception.message}\n#{exception.backtrace&.first(10)&.join("\n")}")
    render_error(status: :internal_server_error, message: "Si è verificato un errore imprevisto")
  end
end
