...
describe('DiscountPercentagePipe', () => {
  ...
  // ─── Casi a esempio ────────────────────────────────────────────────────────
  it('should compute the discount as a rounded percentage', () => {
    expect(pipe.transform(100, 75)).toBe(25);
    expect(pipe.transform(15, 12)).toBe(20);
  });

  it('should return 0 when the price is not lower than the original price', () => {
    expect(pipe.transform(100, 100)).toBe(0);
    expect(pipe.transform(100, 120)).toBe(0);
  });

  it('should return 0 for a non positive original price', () => {
    expect(pipe.transform(0, 50)).toBe(0);
    expect(pipe.transform(-10, 5)).toBe(0);
  });

  it('should accept prices arriving from the backend as strings', () => {
